...
function collectGigDataOnce(){
  ...
  rows.forEach((tr, index)=>{
    ...
    // Strategy 1: Look in second column (typical table layout)
    const td2=tr.querySelector("td:nth-child(2)");
    if (td2){ 
      const a=td2.querySelector("a[href]"); 
      title=clean(a?a.textContent:td2.textContent); 
      url=a?a.href:""; 
      console.log(`[Fiverr Reader] Strategy 1 (td:nth-child(2)) found:`, {title, url});
    }
    ...
    const skipPatterns = /^(impressions|clicks|views|orders|revenue|earnings|status|active|pending|draft|denied|paused)$/i;
  ...
  if (msg.type==="NAV_TO_GIGS_AND_SCRAPE"){
    (async()=>{
      try{
        ...
        sendResponse({status:"OK", gigs, debug: {
          url: location.href,
